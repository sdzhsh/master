...
if aw == 'y':
    ...
    for i in range(n):
        ...
        if pk == 'com.quvideo.slideplus':
            ...
            devicedir = os.path.exists("D:\\git\\monkeyTest-master\\" + name + '-' + model + '-' + dev)
            if devicedir:
                print("File Exist, go on testing!")
            else:
                os.mkdir("D:\\git\\monkeyTest-master\\" + name + '-' + model + '-' + dev)  # 按设备ID生成日志目录文件夹
            wl = open("D:\\git\\monkeyTest-master\\" + name + '-' + model + '-' + dev + '-logcat' + '.cmd', 'w')
            wl.write('adb -s ' + dev + ' logcat' + ' > D:\\git\\monkeyTest-master\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\'+ run_time + '_logcat.txt\n' )
            #wl.write(' > D:\\PyCharm\\monkeyTest-master\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\logcat.txt\n')
            wl.close()
            #if testmodel == '1':
            wd = open("D:\\git\\monkeyTest-master\\" + name + '-' + model + '-' + dev + '-device' + '.cmd', 'w')
            wd.write(
            ...
            wd.write('> D:\\git\\monkeyTest-master\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\' + run_time + '_monkey.txt\n')
